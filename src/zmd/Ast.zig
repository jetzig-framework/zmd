...
pub fn init(allocator: std.mem.Allocator, input: []const u8) Ast {
    return .{
        .allocator = allocator,
        .input = input,
        .tokens_list = (std.ArrayList(tokens.Token).initCapacity(allocator, 0) catch unreachable),
        // .debug = true,
    };
}
...
pub fn tokenize(self: *Ast) !void {
    ...
    try self.tokens_list.append(self.allocator, .{ .element = tokens.Root, .start = 0, .end = 0 });
    ...
    while (index < self.input.len) {
        if (self.firstToken(previous_token, index)) |token| {
            ...
            if (self.tokens_list.items.len == 2 and self.tokens_list.items[1].start > 0) {
    ...
    try self.tokens_list.append(self.allocator, .{
        .element = .{ .type = .eof },
...
fn parseChildNodes(self: *Ast, start: usize, node: *Node) error{OutOfMemory}!bool {
    ...
    while (index < end) {
        ...
        if (try self.parseChildNodes(index, child_node)) {
            if (child_node.token.element.type != .none) try node.children.append(self.allocator, child_node);
        }
    }
    return true;
}
...
fn insertText(self: *Ast, index: usize, start: usize, end: usize) !void {
    try self.tokens_list.insert(self.allocator, index, .{
        .element = tokens.Paragraph,
        .start = start,
        .end = end,
    });

    try self.tokens_list.insert(self.allocator, index + 1, .{
        .element = tokens.Text,
        .start = start,
        .end = end,
    });
...
fn parseText(
        ...
        for (tokens.formatters) |element| {
            if (std.mem.startsWith(u8, input[cursor..], element.syntax)) {
                ...
                if (std.mem.indexOf(u8, input[offset..], element.syntax)) |end_index| {
                    ...
                    try node.children.append(self.allocator, pre_text_node);
                    ...
                    try child_node.children.append(self.allocator, text_node);
    ...
    if (node.children.items.len > 0) {
        ...
        if (last_child.token.end < node.token.end) {
            ...
            try node.children.append(self.allocator, remainder_node);
        }
        node.token = .{
            .element = tokens.Text,
...
fn parseLink(self: *Ast, node: *Node, index: usize, link_type: enum { image, link }) void {
    ...
    if (index + expected_tokens.len + 1 >= self.tokens_list.items.len - 1) return self.swapText(node, index);

    for (expected_tokens, index + 1..) |expected, offset| {
        if (self.tokens_list.items[offset].element.type != expected) return self.swapText(node, index);
    }
...
fn parseBlock(self: *Ast, node: *Node, index: usize) void {
    const token = self.tokens_list.items[index];
    const close_token = if (self.getCloseIndex(index)) |close| self.tokens_list.items[close] else return;
...
fn parseList(
    ...
    while (token_index < self.tokens_list.items.len) {
        ...
        try node.children.append(self.allocator, list_item_node);
...
fn expectTokens(self: Ast, start: usize, expected: []const tokens.ElementType) bool {
    if (start + expected.len >= self.tokens_list.items.len) return false;
...
fn nullifyToken(self: *Ast, index: usize) void {
    self.tokens_list.replaceRangeAssumeCapacity(index, 1, &[_]tokens.Token{.{
        .element = .{ .type = .none },
        .start = self.tokens_list.items[index].start,
        .end = self.tokens_list.items[index].end,
    }});
...
fn createNode(self: Ast, token: tokens.Token) !*Node {
    ...
    node.* = .{
        .token = token,
        .children = (std.ArrayList(*Node).initCapacity(self.allocator, 0) catch unreachable),
    };
    return node;
}
...
fn debugToken(self: Ast, token: tokens.Token) void {
    var buf = std.ArrayList(u8).initCapacity(self.allocator, 0) catch unreachable;
    defer buf.deinit(self.allocator);
    var writer = buf.writer(self.allocator).adaptToNewApi(&.{}).new_interface;
